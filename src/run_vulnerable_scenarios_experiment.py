...
2. Tests them with Gemini API for code generation
3. Uses Claude for vulnerability analysis
4. Generates a report comparing results

Usage:
    python run_vulnerable_scenarios_experiment.py [--model MODEL_NAME]
"""
...
def main():
    """Run the vulnerable scenarios experiment."""
    parser = argparse.ArgumentParser(
        description="Run experiment on 12 vulnerable SQL injection scenarios",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_vulnerable_scenarios_experiment.py
  python run_vulnerable_scenarios_experiment.py --model gemini-2.5-flash
  python run_vulnerable_scenarios_experiment.py --model gemini-1.5-pro
"""
    )
    ...
    print("\n" + "=" * 70)
    print("VULNERABLE SCENARIOS EXPERIMENT")
    print("=" * 70 + "\n")

    print(f"Testing {len(VULNERABLE_SCENARIOS)} scenarios that previously generated vulnerable code")
    print(f"Code Generator: Gemini ({args.model})")
    print(f"Vulnerability Analyzer: Claude\n")
    ...
    # Claude client (required for analysis)
    ...
    base_name = args.name or f"vulnerable_scenarios_{args.model.replace('-', '_')}"
    ...
    runner = CodeCompletionRunner(
        code_generator_client=gemini_client,
